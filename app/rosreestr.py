"""
Кадастровый номер → геометрия участка через ФГИС ЕГРН.

Обёртка над `rosreestr2coord` (https://github.com/rendrom/rosreestr2coord) —
открытая библиотека, ключей не требует.

Три свойства источника определяют весь дизайн вокруг него:

* **он медленный** — ответ занимает секунды, иногда десятки;
* **он режет по IP** — WAF ФГИС отдаёт 403 на запросы из-за пределов РФ и
  из части корпоративных сетей и VPN. С машины разработки поиск недоступен,
  при развёртывании в РФ должен работать;
* **его TLS не проверяется**. `rosreestr2coord` внутри использует
  `ssl._create_unverified_context()`, то есть сертификат сервера не
  валидируется вообще. Своего CA-bundle это не требует (в том числе не
  требует установки корневого сертификата Минцифры в систему), но означает,
  что канал уязвим к MITM. Для нас это приемлемо: наружу не уходит ничего
  секретного, а приходят публичные границы участков, которые всё равно
  проверяет глазами сотрудник ООПТ перед тем, как территория начнёт
  использоваться. Подделать их незаметно нельзя. Ни один другой запрос
  приложения через этот код не идёт.

Резолвинг никогда не выполняется внутри запроса пользователя: участок
сохраняется со статусом `pending`, а границы подтягиваются фоновой задачей.
Не получилось — участок остаётся в списке со статусом `failed` и понятной
причиной, границы вводятся вручную. Терять кадастровый номер из-за того, что
чужой сервис лежит, нельзя.
"""
